...
CONTROL(steering, 5, 9, 127, 100, 155)
...
ISR (PCINT2_vect) {
	...
	if ( new_state ) {
		...
		if (rise_time != 0 && fall_time != 0 && now > fall_time && fall_time > rise_time) {
			...
			// If we're out of passthrough mode and this device has limits, check them.
			if (passthrough == 0) {
				if (
			...
			// If in passthrough mode, set the output pin the value just now read from the input pin.	
			if (passthrough) {
